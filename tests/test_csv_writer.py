...
class TestCSVWorkloadWriter(unittest.TestCase):
    ...
    def test_label_updated_csv_write(self):
        """Test that reading a CSV, generating a trace, and writing back produces the same CSV when labels are used"""
        # Original CSV data
        original_csv = "\n".join([
            "pipeline_id,arrival_seconds,priority,operator_id,parents,baseline_cpu_seconds,cpu_scaling,memory_gb,storage_read_gb,op_type,file_size_mb",
            "p1,0.0,QUERY,op1,,15.0,linear3,,35.0,read,102",
            "p1,,,op2,op1,10.0,sqrt,,20.0,write,204",
            "p2,1.0,BATCH_PIPELINE,op1,,5.0,const,,45.0,read,423",
            "p2,,,op2,op1,8.0,linear7,,25.0,transform,",
            "p2,,,op3,op2,12.0,squared,,30.0,write,75",
            "p3,2.0,INTERACTIVE,op1,,20.0,const,,40.0,read,222"
        ])
        ...
        output_csv = io.StringIO()
        writer = CSVWorkloadWriter(output_csv, label_columns=['op_type', 'file_size_mb'])
        for row in trace_generator.generate_rows():
            writer.write_row(row)
